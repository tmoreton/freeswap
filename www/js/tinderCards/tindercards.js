...
app.controller('CardsCtrl', function($scope, $window, $ionicPopover, TDCardDelegate, AuthService, swipe, user, productFactory, userInfo, cards, chat, $firebaseObject) {
  ...
  $scope.cardSwipedRight = function() {
    ...
    swipe.createMatch($scope.currentCard, $scope.userInfo).then(function(response) {
      $scope.createRoom[response._id] = [{from: 'Free-Swap Admin', message: 'asdf'}];
      $scope.createRoom.$save().then(function(response) {
        // console.log('Firebase saved', response);
